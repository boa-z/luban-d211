...
void show_usage(char *program_name)
{
    ...
    printf("ArtVG Blit Recolor Test - Tests recolor and color key functionality\n\n");
    printf("Options:\n");
    printf("  --recolor-en <0|1>         Enable recolor (default: 0)\n");
    printf("  --recolor-value <0xAARRGGBB>  Recolor value (default: 0xFF00FF00 green)\n");
    printf("  --colorkey-en <0|1>        Enable color key (default: 0)\n");
    printf("  --colorkey-value <0xAARRGGBB> Color key value (default: 0xFF000000 black)\n");
    ...
    printf("\nExamples:\n");
    printf("  Test recolor only:\n");
    printf("    %s --recolor-en 1 --recolor-value 0xFF00FF00\n", program_name);
    printf("  Test color key only:\n");
    printf("    %s --colorkey-en 1 --colorkey-value 0xFF000000\n", program_name);
    printf("  Test both together:\n");
    printf("    %s --recolor-en 1 --colorkey-en 1\n", program_name);
}
...
int main(int argc, char **argv)
{
    ...
    int mirror = 0;
    int recolor_en = 0;
    uint32_t recolor_value = 0xFF00FF00;  /* Default: green */
    int colorkey_en = 0;
    uint32_t colorkey_value = 0xFF000000;  /* Default: black */

    static struct option long_options[] = {
        {"recolor-en", required_argument, 0, 'R'},
        {"recolor-value", required_argument, 0, 'r'},
        {"colorkey-en", required_argument, 0, 'K'},
        {"colorkey-value", required_argument, 0, 'k'},
    ...
    while ((opt = getopt_long(argc, argv, "h", long_options, NULL)) != -1) {
        switch (opt) {
        case 'R':
            recolor_en = atoi(optarg);
            break;
        case 'r':
            recolor_value = strtoul(optarg, NULL, 0);
            break;
        case 'K':
            colorkey_en = atoi(optarg);
            break;
        case 'k':
            colorkey_value = strtoul(optarg, NULL, 0);
            break;
        case 'o':
            rotate = atoi(optarg);
            break;
    ...
    printf("=== ArtVG Blit Recolor Test ===\n\n");
    ...
    ctrl.alpha_en = 0;
    ctrl.src_recolor_en = recolor_en ? 1 : 0;
    ctrl.src_recolor_value = recolor_value;
    ctrl.color_key_en = colorkey_en ? 1 : 0;
    ctrl.color_key_value = colorkey_value;
    ...
    printf("  Rotation: %d (0:0°, 1:90°, 2:180°, 3:270°)\n", rotate);
    printf("  Mirror: %d (0:none, 1:horizontal, 2:vertical)\n", mirror);
    printf("  Recolor: %s (value: 0x%08X)\n",
           recolor_en ? "enabled" : "disabled", recolor_value);
    printf("  Color Key: %s (value: 0x%08X)\n",
           colorkey_en ? "enabled" : "disabled", colorkey_value);
    printf("\n");
