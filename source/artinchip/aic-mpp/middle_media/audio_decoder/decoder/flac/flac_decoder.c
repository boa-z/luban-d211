...
static mpp_sem_t *mpp_sem_create(void)
{
	mpp_sem_t *sem = (mpp_sem_t *)mpp_alloc(sizeof(mpp_sem_t));
	if (!sem)
		return NULL;
	sem->value = 0;
...
static void mpp_sem_wait(mpp_sem_t *sem, int timeout)
{
	(void)timeout;
...
static void mpp_sem_signal(mpp_sem_t *sem)
{
	pthread_mutex_lock(&sem->mutex);
	sem->value++;
...
static void mpp_sem_delete(mpp_sem_t *sem)
{
	if (sem) {
                                  ...
                                  const FLAC__int32 *const buffer[])
{
    ...
    while (i++ < try_cnt) {
        frame = audio_fm_dequeue_empty_frame(flac_decoder->decoder.fm);
        if (frame)
            break;
        usleep(10000);
    }
    ...
    logd("write_pcm frame_id %"PRIu32", blocksize %"PRIu32", pts %"PRId64".", flac_decoder->frame_id,
    ...
    int shift = flac_decoder->bits_per_sample - 16;
    int round_val = shift > 0 ? (1 << (shift - 1)) : 0;
    if (shift < 0)
        shift = 0;
    for (i = 0; i < flac_frame->header.blocksize; i++) {
        /* output sample(s) in 16-bit signed little-endian PCM */
        FLAC__int16 sample0 = (FLAC__int16)((buffer[0][i] + round_val) >> shift);
        data[pos++] = (sample0 >> 0) & 0xff;
        data[pos++] = (sample0 >> 8) & 0xff;
        if (frame->channels == 2) {
            FLAC__int16 sample1 = (FLAC__int16)((buffer[1][i] + round_val) >> shift);
            data[pos++] = (sample1 >> 0) & 0xff;
            data[pos++] = (sample1 >> 8) & 0xff;
        }
    }
    frame->size = pos;
    audio_fm_enqueue_ready_frame(flac_decoder->decoder.fm, frame);
...
int __flac_decode_init(struct aic_audio_decoder *decoder, struct aic_audio_decode_config *config)
{
    ...
    { struct sched_param param; param.sched_priority = 22; pthread_attr_setschedparam(&attr, &param); }
