 ...
 *  Desc: audio packet manager interface
 */
...
int audio_pm_return_empty_packet(struct audio_packet_manager *pm, struct mpp_packet *packet)
{
	struct audio_packet_impl *pkt_impl;
	...
	if (pkt_impl->pos_offset > 0) {
		...
		pm->write_offset = (unsigned char *)pkt_impl->pkt.data - pm->buffer_start;
		pm->available_size += pkt_impl->pos_offset + pkt_impl->pkt.size;
	}
