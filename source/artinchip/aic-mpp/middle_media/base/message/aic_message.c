...
s32 aic_msg_put(struct aic_message_queue *msg_que, struct aic_message *msg)
{
	...
	message = mpp_list_first_entry(&msg_que->empty_msg_list,
				       struct aic_message, list);
	...
	logv("queue node:[%d], queue msg cnt:[%d], msg_id:[%d], "
	     "para0:[%d], data_size:[%d], data:[%p]\n",
	     msg_que->msg_node_cnt, msg_que->msg_cnt,
	     message->message_id, message->param,
...
s32 aic_msg_get(struct aic_message_queue *msg_que, struct aic_message *msg)
{
	...
	message = mpp_list_first_entry(&msg_que->ready_msg_list,
				       struct aic_message, list);
...
s32 aic_msg_wait_new_msg(struct aic_message_queue *msg_que, u64 us)
{
	...
	if (us == 0) {
		...
		msg_que->need_signal = 1;
		ret = pthread_cond_timedwait(&msg_que->msg_cond,
					     &msg_que->mutex, &out_time);
		msg_que->need_signal = 0;
	}
	pthread_mutex_unlock(&msg_que->mutex);
