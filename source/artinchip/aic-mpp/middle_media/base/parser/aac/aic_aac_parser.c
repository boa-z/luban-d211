...
static s32 parse_adts_header(struct aic_aac_parser *parser, ADTSHeader *header, u8 *data)
{
    ...
    if (header->syncword != 0xFFF) {
        loge("Invalid ADTS syncword: 0x%x, tell: 0x%llx", header->syncword, (long long unsigned int)aic_stream_tell(parser->stream));
...
s32 aac_peek(struct aic_parser *parser, struct aic_parser_packet *pkt)
{
    ...
    pkt->dts = 0;
    int sampling_freq = sampling_freq_table[header.sampling_freq_index];
    if (0 == sampling_freq) {
        loge("sampling_freq is 0");
        return PARSER_ERROR;
    }
    pkt->duration = (1024 * 1000000) / sampling_freq; // 1024 samples per frame
    pkt->type = MPP_MEDIA_TYPE_AUDIO;
    if ((cur_pos + header.frame_length) >= aac_parser->file_size)
        pkt->flag = PACKET_FLAG_EOS;
    else
        pkt->flag = 0;
    logi("peek packet, size: %d, tell: %llx", pkt->size, (long long unsigned int)aic_stream_tell(aac_parser->stream));
...
{
    if (!pkt || !pkt->data) {
        loge("packet or packet data is null");
        return PARSER_ERROR;
    }
...
s32 aac_get_media_info(struct aic_parser *parser, struct aic_parser_av_media_info *media)
{
    ...
    media->has_video = 0;
    media->has_audio = 1;
    media->audio_stream[0].bits_per_sample = aac_parser->media_info.audio_stream[0].bits_per_sample;
...
s32 aac_init(struct aic_parser *parser)
{
    ...
    // Audio stream info
    aac_parser->media_info.audio_stream[0].codec_type = MPP_CODEC_AUDIO_DECODER_AAC;
    aac_parser->media_info.audio_stream[0].sample_rate = sampling_freq_table[header.sampling_freq_index];
    aac_parser->media_info.audio_stream[0].nb_channel = header.channel_config; //faad dec channel is 2, header.channel_config;
    aac_parser->media_info.audio_stream[0].bits_per_sample = 16; // AAC is typically 16-bit
    aac_parser->media_info.audio_stream[0].bit_rate = 0; // Can be calculated if duration is known
    aac_parser->media_info.audio_stream[0].extra_data_size = 0;
    // Restore position
    aic_stream_seek(aac_parser->stream, current_pos, SEEK_SET);
