...
s32 ape_peek(struct aic_parser *parser, struct aic_parser_packet *pkt)
{
	...
	if (!parser || !pkt) {
		loge("ape_peek: invalid parameter\n");
		return PARSER_ERROR;
	}
...
s32 ape_read(struct aic_parser * parser, struct aic_parser_packet *pkt)
{
	...
	if (!parser || !pkt) {
		loge("ape_read: invalid parameter\n");
		return PARSER_ERROR;
	}
...
s32 ape_get_media_info(struct aic_parser *parser, struct aic_parser_av_media_info *media)
{
	...
	if (!parser || !media) {
		loge("ape_get_media_info: invalid parameter\n");
		return -1;
	}
	...
	media->audio_stream[0].codec_type = MPP_CODEC_AUDIO_DECODER_APE;
	media->audio_stream[0].bits_per_sample = 16;
	media->audio_stream[0].nb_channel = ape_parser->ape_ctx.channels;
	media->audio_stream[0].sample_rate = ape_parser->ape_ctx.samplerate;
