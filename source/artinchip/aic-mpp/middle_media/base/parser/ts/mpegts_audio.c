...
int mpegaudio_decode_aac_header(uint8_t *data, struct mpegts_audio_decode_header *s,
                                int is_gen_seq_header, uint8_t *seq_header_data)
{
    int frame_size;
    ...
    if (ret < 0) {
        loge("aac_check_header failed, header: 0x%x", header);
        return ret;
    }
    ...
    chn_config = (header >> 6) & 0x7; // Take 3 bits starting from the 6th position of the header.
