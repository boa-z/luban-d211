...
s32 flac_get_media_info(struct aic_parser *parser, struct aic_parser_av_media_info *media)
{
    ...
    for (i = 0; i < c->nb_streams; i++) {
        ...
        if (st->codecpar.codec_type == MPP_MEDIA_TYPE_AUDIO) {
            media->has_audio = 1;
            media->audio_stream[0].codec_type = MPP_CODEC_AUDIO_DECODER_FLAC;
            media->audio_stream[0].bits_per_sample =
                st->codecpar.bits_per_coded_sample;
            media->audio_stream[0].nb_channel = st->codecpar.channels;
            media->audio_stream[0].sample_rate = st->codecpar.sample_rate;
            if (st->codecpar.extradata_size > 0) {
                media->audio_stream[0].extra_data_size =
                    st->codecpar.extradata_size;
                media->audio_stream[0].extra_data = st->codecpar.extradata;
            }
