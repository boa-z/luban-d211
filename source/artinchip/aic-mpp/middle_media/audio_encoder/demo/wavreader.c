...
int parse_wav_header(struct wav_reader* wr, uint32_t *length)
{
    if ((NULL == wr) || (NULL == length)) {
        return -1;
    }
    ...
    if (wr->format == 0xfffe) {
        ...
        skip(wr->wav, *length - 28);
    } else {
        skip(wr->wav, *length - 16);
    }
