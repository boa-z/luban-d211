...
struct aic_player_ctx {
...
#ifdef AIC_PLAYER_UI_RENDER_SUPPORT
    /* mjpeg */
    lv_ll_t frame_ll;
    ...
    void *image_src;
    backend_screen_info_t screen_info;
...
static lv_res_t aic_player_backend_set_src(void *ctx, const char *src)
{
    ...
    if (aic_ctx->draw_layer == LV_AIC_PLAYER_LAYER_VIDEO) {
        lv_image_t *image = (lv_image_t *)player_ctx->obj;
        image->w = aic_ctx->media_info.video_stream.width;
        image->h = aic_ctx->media_info.video_stream.height;
...
#endif /* AIC_PLAYER_UI_RENDER_SUPPORT */
