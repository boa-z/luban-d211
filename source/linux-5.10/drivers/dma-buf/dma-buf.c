...
static int dma_buf_sync_range(struct dma_buf_range *range)
{
	if (IS_ERR_OR_NULL((void *)range->start) ||
			IS_ERR_OR_NULL((void *)(range->start + range->size))) {
		pr_info("flush 0x%llx, size %llu fault user virt address!\n",
	...
	if (range->flags & DMA_BUF_SYNC_WB_RANGE)
		...
		dma_usr_va_inv_range((void *)range->start, (unsigned long)range->size);
