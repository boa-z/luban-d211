		...
		u32 lane, u32 vc, struct videomode *vm)
{
	...
	u32 ht = vm->hback_porch + vm->hsync_len + vm->hactive +
		vm->hfront_porch;
	...
	reg_set_bits(base + DSI_VID_VBLANK_LINE,
		...
		DSI_VID_VBP_TIME(vm->vback_porch));
	reg_set_bits(base + DSI_VID_VBLANK_LINE,
		DSI_VID_VFP_LINE_MASK,
		DSI_VID_VFP_LINE(vm->vfront_porch));
	reg_set_bits(base + DSI_VID_VACT_LINE,
		DSI_VID_VACT_TIME_MASK,
		DSI_VID_VACT_TIME(vm->vactive));
}
