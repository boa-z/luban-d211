			      ...
			      struct spl_image_info *image_info)
{
	...
	if (fit_image_get_load(fit, node, &load_addr)) {
		if (!image_info->load_addr) {
			pr_err("Can't load %s: No load address and no buffer\n",
...
static int spl_fit_image_get_os(const void *fit, int noffset, uint8_t *os)
{
	...
	else if (!strcmp(name, "opensbi"))
		*os = IH_OS_OPENSBI; /* for space saving */
	else if (!strcmp(name, "artinchip"))
		*os = IH_OS_ARTINCHIP; /* for space saving */
#endif
	else
		*os = IH_OS_INVALID;
...
extern void spl_opensbi_set_uboot_entry(ulong entry);
#endif
			...
			struct spl_load_info *info, ulong sector, void *fit)
{
	...
	/* Now check if there are kernel for us to load.
	 * If kernel is exist, it is in Falcon mode.
	 */
	...
	if (!spl_start_uboot()) {
		node = 0;
		node = spl_fit_get_image_node(&ctx, FIT_KERNEL_PROP, 0);
		if (node >= 0 && firmware_node != node) {
			uint8_t os = IH_OS_INVALID;
			/*
			 * In falcon mode, first loading the kernel to the load_addr
			 * second, check if the os is linux ,then loading the fdt to
			...
			if (!spl_fit_image_get_os(ctx.fit, node, &os))
...
falcon_end:
	...
	for (; ; index++) {
		...
		if (ret < 0) {
			pr_err("%s: can't load image loadables index %d (ret = %d)\n",
			       __func__, index, ret);
			return ret;
		}
