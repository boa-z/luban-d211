...
static void aic_ehci_set_phy_type(struct ehci_hccr *hccr, int phy_type)
{
	u32 val = 0;
	unsigned long regs = (unsigned long)hccr;

	val = ehci_readl((void *)(regs + USB_HOST_CTRL_REG_OFFSE));

	if (phy_type == PHY_TYPE_ULPI) {
		ehci_writel((void *)(regs + USB_HOST_CTRL_REG_OFFSE),
			    val & ~ULPI_BYPASS_EN);
	} else {
		ehci_writel((void *)(regs + USB_HOST_CTRL_REG_OFFSE),
			    val | ULPI_BYPASS_EN);
...
static int ehci_usb_probe(struct udevice *dev)
{
	...
	hccr = map_physmem(dev_read_addr(dev), 0x1000, MAP_NOCACHE);
