		     ...
		     char *const argv[])
{
	...
	io_op.len = len;
	if (has_pages) {
		/* NAND, first read size should be in current block */
		io_op.len = (mtd->erasesize - off % mtd->erasesize);
		if (io_op.len > len)
			io_op.len = len;
	}
	...
	while (remaining) {
		...
		io_op.len = remaining;
		if (has_pages && (remaining > mtd->erasesize))
